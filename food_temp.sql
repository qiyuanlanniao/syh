...
CREATE TABLE `food`  (
  ...
  PRIMARY KEY (`id`)
) ENGINE = InnoDB DEFAULT CHARACTER SET utf8 COMMENT = '食品表';
...
) ENGINE = InnoDB DEFAULT CHARACTER SET utf8 COMMENT = '食品分类';
...
CREATE TABLE `food_sale_change_log`  (
  ...
  PRIMARY KEY (`id`),
  INDEX `idx_food_id_id`(`food_id`)
) ENGINE = InnoDB DEFAULT CHARACTER SET utf8 COMMENT = '商品销售情况';
...
) ENGINE = InnoDB DEFAULT CHARACTER SET utf8 COMMENT = '数据库存变更表';
...
CREATE TABLE `member`  (
  ...
  `created_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '插入时间',
  PRIMARY KEY (`id`)
) ENGINE = InnoDB DEFAULT CHARACTER=utf8 COMMENT = '会员表';
...
) ENGINE = InnoDB DEFAULT CHARACTER=utf8 COMMENT = '第三方登录绑定关系';
...
CREATE TABLE `pay_order`  (
  ...
  INDEX `idx_member_id_status`(`member_id`, `status`) USING BTREE
) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COMMENT = '在线购买订单表';
...
CREATE TABLE `pay_order_callback_data`  (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `pay_order_id`(`pay_order_id`)
...
ALTER TABLE `user` COMMENT '用户表（管理员）';
...
CREATE TABLE `wx_share_history`  (
  ...
  `created_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COMMENT = '微信分享记录';
